...
float SpectralContrast::roundf(float x)
{
...
SpectralContrast::process(const float *const *inputBuffers, Vamp::RealTime timestamp)
{
  ...
  for (int band=0; band<numBands; band++)
  {
    ...
    int start = 0;
    int end = 1;
    if (bins.at(band).size() >= (1/alpha)) end = roundf(bins.at(band).size()*alpha);
    float valleySum = 0;
    ...
    if (bins.at(band).size() >= (1/alpha)) start = bins.at(band).size() - roundf(bins.at(band).size()*alpha);
